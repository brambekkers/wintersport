                ...
                <div :key="i" :class="header.class">
                    ...
                    <v-list-item-group v-if="header.show" color="primary">
                        ...
                        <template v-if="!header.subItems">
                            <MenuItem v-for="(item, j) in header.items" :key="j" :title="item.title" :icon="item.icon" :action="item.action" :route="item.route" />
                        </template>

                        <!-- SubMenu -->
                        <template v-if="header.subItems">
                            <v-list-group :value="true" v-for="(subItem, j) in header.subItems" :key="j" :prepend-icon="subItem.icon">
                                <template v-slot:activator>
                                    <v-list-item-title>{{ subItem.name }}</v-list-item-title>
                                </template>
                                <MenuItem v-for="(item, j) in subItem.items" :key="j" :title="item.title" :icon="item.icon" :action="item.action" :route="item.route" />
                            </v-list-group>
                        </template>
                    </v-list-item-group>
